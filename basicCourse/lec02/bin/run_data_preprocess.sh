...
biclass_train_fp="$DATADIR/${POSLAB}_train"
biclass_test_fp="$DATADIR/${POSLAB}_test"
biclass_valid_fp="$DATADIR/${POSLAB}_validate"
...
function gen_biclassData() {
    ...
    cat $1| egrep -v "^${POSLAB}\t" \
        | python $sample_bin $pos_sample_cnt > $tmp_neg_fp
